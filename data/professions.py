...
class ProfessionsData:
    ...
    def get_dataloaders(self, batch_size, shuffle=True, val_split=0.1):
        train_data, test_data = train_test_split(
            self.data, test_size=val_split, random_state=self.seed
        )
    ...
    def get_stereo(self):
        ...
        with open(
            path.join(self.data_path, "professions_female_stereo_nl_corr2.json"), "r"
        ) as f:
            for l in f:
                for j in eval(l):
    ...
    def get_profession_list(self, indices=None):
        word_list = []
        with open(path.join(self.data_path, "professions_nl.json"), "r") as f:
            for l in f:
                for j in eval(l):
    ...
    def get_template_list(self, indices=None):
        ...
        templates = [
            ...
            "De {} huilde omdat",
            "De {} lachte omdat",
            "De {} ging naar huis omdat",
            "De {} bleef wakker omdat",
            "De {} was ontslagen omdat",
            "De {} was gepromoveerd omdat",
            "De {} schreeuwde omdat",
        ]
        if indices:
...
if __name__ == "__main__":
    ...
    train_dataloader, val_dataloader = data.get_dataloaders(
        batch_size=1, shuffle=True, val_split=0.00001
    )
    m, f = data.get_stereo()
    print(len(m))
    print(len(f))
