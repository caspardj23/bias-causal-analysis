...
def box_plot(df, figure_path, score_column, y_label):
    ...
    fig.add_scatter(
        ...
        xaxis="x2",
        showlegend=False,
        line=dict(dash="dash", color="#8E44AD"),
    )
...
def plot_results():
    ...
    box_plot(
        results,
        "results/plots/crows_pairs.pdf",
        "CrowS-Pairs",
        "CrowS-Pairs Stereotype Score",
    )
    box_plot(
        results,
        "results/plots/professions.pdf",
        "Professions",
    ...
    results_stereoset = results.select(
        [
            "model",
            "seed",
            "model_type",
    ...
    results_blimp = results_blimp.select(
        [
            "model",
            "seed",
            "model_type",
    ...
    box_plot(
        results_blimp, "results/plots/blimp_aga.pdf", "BLiMP AGA", "BLiMP AGA Score"
    )
    ...
    results_winobias = results.select(
        [
            "model",
            "seed",
            "model_type",
    ...
    results_winobias = results_winobias.with_columns(
        [
            ((pl.col("WinoBias Type1 Dev") + pl.col("WinoBias Type1 Test")) / 2).alias(
                "WinoBias1"
            ),
            ((pl.col("WinoBias Type2 Dev") + pl.col("WinoBias Type2 Test")) / 2).alias(
                "WinoBias2"
            ),
            (
                (
                    pl.col("WinoBias Type1 Dev")
                    + pl.col("WinoBias Type1 Test")
                    + pl.col("WinoBias Type2 Dev")
                    + pl.col("WinoBias Type2 Test")
                )
                / 4
            ).alias("WinoBias"),
        ]
    )
    results_winobias = results_winobias.select(
        [
            "model",
            "seed",
            "model_type",
    ...
    box_plot(
        results_winobias,
        "results/plots/winobias.pdf",
        "WinoBias",
        "WinoBias Overall Score",
    )
    ...
    box_plot(
        results_winobias,
        "results/plots/winobias1.pdf",
        "WinoBias1",
        "WinoBias Type1 Score",
    )
...
def plot_attn_heads():
    ...
    pio.write_image(
        fig,
        Path("results/plots") / f"heads_cma.png",
        format="png",
        width=1000,
        height=1000,
    )
    ...
    diffmask = (
        Path("results/discovery")
        / "gpt2-small_attn_heads_10_mlps_0_epochs_200_lr_0.001_seed_9.json"
    )
    fig, dm_heads = plot_diffmask_heads(diffmask, 7)
    pio.write_image(
        fig,
        Path("results/plots") / f"heads_dm.png",
        format="png",
        width=1000,
        height=1000,
    )
    ...
    pio.write_image(
        fig,
        Path("results/plots") / f"heads_acdc.png",
        format="png",
        width=1000,
        height=1000,
    )
    ...
    fig.add_trace(
        go.Heatmap(z=cma_heads, colorscale="Blues", showscale=False), row=1, col=1
    )

    fig.add_trace(
        go.Heatmap(z=dm_heads, colorscale="Blues", showscale=False), row=1, col=2
    )

    fig.add_trace(
        go.Heatmap(z=acdc_heads, colorscale=["#8E44AD", "#2ECC71"], showscale=False),
        row=1,
        col=3,
    )
...
def plot_attn_heads_overlap():
    ...
    for l in acdc_heads.keys():
        for h in acdc_heads[l]:
            if (
                l in cma_heads.keys()
                and h in cma_heads[l]
                and l in dm_heads.keys()
                and h in dm_heads[l]
            ):
                overlap[l].append(h)
...
def print_latex(
    df,
    label,
    caption=f"Comparision of different models on the CrowS-Pairs, StereoSet, BLiMP and WinoBias datasets.",
):
    cmap = "RdYlGn"
    latex = (
