...
def save_results(results, model_name, seed):
    with open(
        Path(
            f"/content/drive/My Drive/Mitigation_data/yhavinga_2_21_jul/Evaluate/{model_name}_{seed}.json"
        ),
        "w",
    ) as f:
        json.dump(results, f)
...
def evaluate_cma():
    ...
    data_config = DataConfig(
        path="data", name="professions", workers=4, batch_size=64, val_size=0.15
    )
    ...
    train_dataloader, val_dataloader = professions.get_dataloaders(
        batch_size=cma_config.data.batch_size,
        shuffle=True,
        val_split=cma_config.data.val_size,
    )
    dm_ie = cma.indirect_effect(
        train_dataloader, torch.tensor(dm_mask, dtype=torch.float32).to(device)
    )
    print(f"DM indirect effect: {dm_ie}")
    cma_ie = cma.indirect_effect(
        train_dataloader, torch.tensor(cma_mask, dtype=torch.float32).to(device)
    )
